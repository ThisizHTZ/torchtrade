...
def test_local_backtest_run_all_writes_research_artifacts(tmp_path: Path) -> None:
    ...
    assert len(policy["leaderboard"]) > 1
    assert evaluation["evaluation_metrics"]["bars"] > 0
    assert evaluation["evaluation_metrics"]["final_equity"] > 0
    assert {"cash", "buy_and_hold"}.issubset(evaluation["baselines"])
    assert len(report["walk_forward"]) == 3
    assert report["metadata"]["data_sha256"]
    assert report["metadata"]["config"]["cost_bps"] == 2.0
